...
- compiler: TCP via IPv6
- compiler: Allow embedded payloads for TCP and UDP
- build: build options for profiling (WITH_PROFILE)
...
- compiler: LLDP support. Supported TLVs: IEEE 802.1AB-2016, IEEE 802.1Q-2022, IEEE802.3-2023, Profinet
...
- compiler: Random numbers can have range restrictions. E.g. '*[12-24]'
- compiler: basic datatypes for raw protocol. Packets can be set up via generic building blocks, like integer data types, IP addresses and others.
...
- test: Redesign of ctest environment with YAML-based testcase definition
...
- compiler: fixed oops if value is omitted on stream parameters
...
## Added
- core: Configurable MTU (--mtu option)
- core: IP- and MAC addresses can be defined as random values in packets (e.g. smac=*)
- compiler: Support for recursive packet definitions. Means, a packet can be defined as a payload of another packet (via <> operator). This can be useful for encapsulation protocols like vxlan and GRE or for embedded payloads of ICMP packets.
...
- core: Overwrite destination MAC address of all packets to the MAC specified via new option --overwrite-dmac
