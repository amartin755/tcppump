...
The input file contains a list of ethernet packets in an human readable format. Each packet definition has to be terminated with semicolon.
...
* `timestamp` relative time as integer (us since beginning of the file) e.g. 123456 if time starts with '+', time is relative to the previous packet; e.g. +100
* `protocol` protocol specifier followed by ':', see PROTOCOLS
* `parameters` protocol parameters (format depends on protocol specifier)
              Each parameter name starts with '.' followed by '=' and the value of the parameter itself
  comments start with #

examples:

	+1234   protocol_x: .parameter_x = 3333333333333333333333333333333333333333;
	1000000 protocol_x: .parameter_x = 3333333333333333333333333333333333333333;
...
#### examples
	...
	arp: .dip=11.22.33.44; # who has 11.22.33.44?  Tell IP of host
	# arp-probe as specified in rfc 5227; same as "who has" obove, but sender IP set to zero
